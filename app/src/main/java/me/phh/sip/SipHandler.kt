...
class SipHandler(
    ...
    fun handleUpdate(request: SipRequest): Int {
        ...
        val allTracks = listOf(call.pcmaTrack, call.dtmfTrack).sorted()
        val mySdp =
            """
v=0
...
b=RS:0
b=RR:0
a=rtpmap:${call.pcmaTrack} PCMA/8000
...
a=sendrecv
    ...
    fun callEncodeThread() {
        ...
        thread {
            ...
            Rlog.d(TAG, "Encode thread started: pcmaTrack=${call.pcmaTrack} remote=${call.rtpRemoteAddr}:${call.rtpRemotePort} gen=$gen")
            val encoder = MediaCodec.createEncoderByType("audio/g711-alaw")
            val mediaFormat = MediaFormat.createAudioFormat("audio/g711-alaw", 8000, 1)
            ...
            while (!callStarted.get()) {
                ...
                val pcmaNothing = listOf(0x77, 0xc0) // CMR = 12.2kbps, F=0, FT=15=No TX/No RX, Q=1

                val buf = (rtpHeader + pcmaNothing).map { it.toUByte() }.toUByteArray().toByteArray()
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    while (bufPos < outBufInfo.size) {
                        ...
                        // Encoder outputs octet-aligned PCMA frames (RFC 4867 §5):
                        ...
                        val rtpHeader =
                            byteArrayOf(
                                0x80.toByte(),
                                ((if (firstPacket) 0x80 else 0) or call.pcmaTrack).toByte(),
    ...
    1. Send INVITE with SDP:
         ...
         Lists all tracks we support (PCMA, DTMF).
    ...
    fun call(phoneNumber: String) {
        thread {
            ...
            val allTracks = listOf(pcmaTrack, dtmfTrack).sorted()
...
a=sendrecv
    ...
    fun callDecodeThread() {
        ...
        thread {
            ...
            val decoder = MediaCodec.createDecoderByType("audio/g711-alaw")
            val mediaFormat = MediaFormat.createAudioFormat("audio/g711-alaw", 8000, 1)
    ...
    fun handleCall(request: SipRequest): Int {
        ...
        // Look for an PCMA/8000 mode
        val (pcmaTrack) = lookTrackMatching("PCMA/8000")!!

        // Look for a DTMF track, use the 8000Hz-based one to match PCMA timestamps
